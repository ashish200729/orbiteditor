...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: { tooltipName: string, color?: IndicatorColor, position?: PlacesType, offset?: number }) => ({
	'data-tooltip-id': color === 'orange' ? `orbit-tooltip-orange` : color === 'green' ? 'orbit-tooltip-green' : 'orbit-tooltip',
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`Orbit Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`Orbit Error: There was a problem running Apply: ${e}.`)
...
export const EditToolAcceptRejectButtonsHTML = ({
	...
	uri: URI,
	type: 'StrReplace' | 'Write' | 'edit_file' | 'rewrite_file',
	threadId: string,
})
) => {
	const accessor = useAccessor()
